...
function TokenSelector({ currency, onSetToken, disabledToken }: { currency: string; onSetToken: Dispatch<SetStateAction<string>>; disabledToken: string }) {
      ...
      <SelectTrigger className='basis-3/4 bg-background sm:basis-1/2'>
        <SelectValue aria-label={currency}>
          <span className='flex items-center gap-x-2'>
            <Image src={currentToken?.logoURI as string} alt={`${currentToken?.symbol} Logo`} width={22} height={22} />
            <span className='text-lg font-semibold'>{currentToken?.symbol}</span>
          </span>
        </SelectValue>
      </SelectTrigger>
      <SelectContent className='max-h-[13rem]'>
        {getTokenList.map((token) => (
          <SelectItem value={token.symbol} key={token.symbol} disabled={token.symbol === disabledToken}>
            <span className='flex items-center gap-x-2'>
              <Image src={token.logoURI} alt={`${token.symbol} Logo`} width={22} height={22} />
...
export default function AddInputSelector({
  value,
  onAmountChange,
  currency,
  onSetToken,
  ...
  return (
    <div className='rounded-lg bg-accent p-4 dark:bg-accent/30'>
      <div className='flex items-center gap-x-3 sm:gap-x-0'>
        ...
        />
        <TokenSelector currency={currency} onSetToken={onSetToken} disabledToken={disabledToken} />
      </div>
