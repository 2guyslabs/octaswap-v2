...
export default function useSwap(amountAinput: bigint, amountBInput: bigint, tokenA: Token, tokenB: Token, amountsIn: bigint, amountsOut: bigint) {
  ...
  const { data: swapExactTokensForTokensConfig } = useSimulateContract({
    ...
    args: [amountAinput, amountsOut, path, address as `0x${string}`, BigInt(deadline)],
    query: {
  ...
  const { data: swapTokensForExactTokensConfig } = useSimulateContract({
    ...
    args: [amountBInput, amountsIn, path, address as `0x${string}`, BigInt(deadline)],
    query: {
      enabled: tokenB?.symbol !== 'OCTA' && tokenB?.symbol !== 'WOCTA',
    },
  })
  ...
  const swapConfig = swapExactTokensForTokensConfig || swapTokensForExactTokensConfig || exactEthforTokensConfig || tokensForExactEthConfig || exactTokensforEthConfig || ethForExactTokensConfig
